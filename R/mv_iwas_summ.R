...
mv_iwas_summ <- function(betaZY, se_betaZY, betaZX, se_betaZX, corr_mat, n, trait_type, n_case = NULL, n_control = NULL){
  ...
  if(sum(c(missing(betaZY), missing(betaZX), missing(se_betaZY), missing(se_betaZX), missing(corr_mat), missing(n), missing(trait_type))) > 0){
    ...
    cat("betaZY must be a matrix", "\n")
  }else if(ncol(betaZY) != 1){
    cat("betaZY must be a matrix with only 1 column", "\n")
  }else if(class(se_betaZY) != "matrix"){
    cat("se_betaZY must be a matrix", "\n")
    ...
    se_egger <- matrix(sqrt(diag(solve(t(betaZX_egger) %*% corr_mat %*% betaZX_egger) * c(sigma_egger))), ncol = 1)
